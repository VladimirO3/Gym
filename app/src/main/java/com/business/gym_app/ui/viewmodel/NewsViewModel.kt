...
class NewsViewModel(
    ...
    /**
     * Редактирование существующей новости на VPS сервере.
     */
    fun updateLocalNews(
        context: Context,
        id: String,
        ...
        Log.d("NewsViewModel", "Starting updateLocalNews. ID: $id, Title: $title")
        _isUploading.value = true
        viewModelScope.launch {
            try {
                // Если выбран новый файл, определяем его тип, иначе оставляем старый
                val typeValue = if (uri != null) {
                    val mimeType = context.contentResolver.getType(uri) ?: ""
                    if (mimeType.contains("video")) "video" else "image"
                } else existingType

                // Формируем Multipart тело файла (если выбран новый)
                var filePart: MultipartBody.Part? = null
                if (uri != null) {
                    val contentResolver = context.contentResolver
                    val fileName = getFileName(context, uri) ?: "update_file"
                    val mimeType = contentResolver.getType(uri) ?: "application/octet-stream"
                ...
                // Запрос к репозиторию для обновления на сервере
